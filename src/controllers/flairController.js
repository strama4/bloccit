...
module.exports = {
    ...
    create(req, res, next) {
        ...
        flairQueries.addFlair(newFlair, (err, flair) => {
            if (err) {
                res.redirect(500, `/topics/${flair.topicId}/flairs/new`);
            } else {
    ...
    destroy(req, res, next) {
        flairQueries.deleteFlair(req.params.id, (err, deletedCount) => {
            if (err) {
    ...
    update(req, res, next) {
        flairQueries.updateFlair(req.params.id, req.body, (err, flair) => {
            if (err || flair == null) {
                ...
                res.redirect(302, `/topics/${flair.topicId}/flairs/${flair.id}`)
            }
        })
    }
